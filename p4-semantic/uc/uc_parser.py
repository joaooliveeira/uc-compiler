...
class UCParser:
    ...
    def _fix_decl_name_type(self, decl, typename):
        ...
        if not typename:
            ...
            type.type = Type(typename.names, coord=typename.coord)
    ...
    precedence = (
        ("left", "COMMA"),
        ("right", "TIMESEQUAL", "DIVEQUAL", "MODEQUAL"),
    ...
    def p_identifier_list_or_empty(self, p):
    ...
    def p_identifier_list(self, p):
        ...
        """
        p[0] = [p[1]] if len(p) == 2 else ParamList(p[1] + [p[3]], coord=p[1][0].coord)

    def p_function_definition(self, p):
        """function_definition : type_specifier declarator declaration_list compound_statement
        | empty declarator declaration_list compound_statement
        ...
        p[0] = FuncDef(
            spec,
            self._build_declarations(spec, decls=[dict(decl=p[2], init=None)]),
            p[3],
            p[4],
        )
    ...
    def p_declarator(self, p):
        ...
        | declarator LBRACKET constant_expression_or_empty RBRACKET
        | declarator LPAREN parameter_list RPAREN
        | declarator LPAREN identifier_list_or_empty RPAREN
        """
    ...
    def p_constant_expression_or_empty(self, p):
        """constant_expression_or_empty : empty
        | constant_expression
        """
        p[0] = p[1]
    ...
    def p_postfix_expression(self, p):
        ...
        | postfix_expression LPAREN argument_expression_list_or_empty RPAREN
    ...
    def p_argument_expression_list_or_empty(self, p):
        """argument_expression_list_or_empty : empty
    ...
    def p_parameter_list(self, p):
        ...
        if len(p) == 2:
            ...
            p[0] = ParamList(p[1].params + p[3])

    def p_parameter_declaration(self, p):
        """ parameter_declaration : type_specifier declarator """
        p[0] = self._build_declarations(p[1], [dict(decl=p[2], init=None)])

    def p_declaration(self, p):
        """ declaration : type_specifier init_declarator_list_or_empty SEMI """
        if p[2] is None:
            p[0] = self._build_declarations(p[1], [dict(decl=None, init=None)])
        else:
            p[0] = self._build_declarations(p[1], p[2])

    def p_declaration_list_or_empty(self, p):
        """declaration_list_or_empty : empty
        | declaration_list
        """
        p[0] = p[1]
    ...
    def p_init_declarator_list_or_empty(self, p):
        """init_declarator_list_or_empty : empty
    ...
    def p_initializer_list(self, p):
        ...
        if len(p) == 2:
            ...
            if not isinstance(p[1], InitList):
                p[1] = InitList([p[1]], coord=self._token_coord(p, 1))
            p[1].exprs.append(p[3])
            p[0] = p[1]

    def p_compound_statement(self, p):
        """ compound_statement : LBRACE declaration_list_or_empty statement_list_or_empty RBRACE """
    ...
    def p_statement_list_or_empty(self, p):
    ...
    def p_expression_statement(self, p):
        """ expression_statement : expression_or_empty SEMI """
        if p[1] is None:
    ...
    def p_iteration_statement(self, p):
        ...
        | FOR LPAREN expression_or_empty SEMI expression_or_empty SEMI expression_or_empty RPAREN statement
        | FOR LPAREN declaration expression_or_empty SEMI expression_or_empty RPAREN statement
        """
        if len(p) == 6:
            ...
            p[0] = For(
                DeclList(p[3], coord=self._token_coord(p, 1)),
                p[4],
                p[6],
                p[8],
    ...
    def p_print_statement(self, p):
        """ print_statement : PRINT LPAREN argument_expression_list_or_empty RPAREN SEMI """
